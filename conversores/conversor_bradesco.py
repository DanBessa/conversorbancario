...
def extract_pdf_to_text():
    ...
    output_path = os.path.join(os.path.dirname(pdf_path), "bradesco.txt")
    try:
...
def txt_to_xls(input_file: str, output_file: str):
    ...
    for line in lines:
        ...
        row_data = [current_line_date, historico] + valores[:3]
        ws.append(row_data)
...
def main():
    ...
    # A função extract_pdf_to_text já retorna o caminho do arquivo .txt
    file_path = extract_pdf_to_text()
    if not file_path:
        return # Retorna None se o usuário cancelar, o que está certo.

    # As etapas de processamento do arquivo de texto intermediário
    ...
    nome_base, _ = os.path.splitext(file_path) # Remove a extensão .txt
    caminho_final_excel = nome_base + ".xlsx"
    txt_to_xls(file_path, caminho_final_excel)
